...
class RandomReplacement(n_ways: Int) extends ReplacementPolicy {
  ...
  def way = Random(n_ways, lfsr)%1.U(n_ways.W)
...
//This is RRIP Policy named as TrueLRU. To run RRIP uncomment this part of code and comment all the other classes name TrueLRU

class TrueLRU(n_ways: Int) extends ReplacementPolicy {
  // Static re-reference interval policy, using a 2bits per way to track near-immediate and distant re-reference block
  //The bits are filled from MSB to LSB [way-1 way-2 way-3 way-4]
  // [01 10 10 10] - In this case, max RRPV is of 2 so first all will be increased by one to get [10 11 11 11] then way-2 will be replaced
  // [10 01 11 11] - This is the next state from above case, for next replacement way-3 and way-4 will be sected in the same order
  // [10 01 01 01] - Now all have to be updated as no distant re-reference way. [11 10 10 10] after this way-1 will be replaced
  // [01 10 10 10] - Now if way-2 gets a hit then it will bdecremented [01 01 10 10].
  // [10 10 01 11] - Next replace will increase all by 1 and way-3 will be replaced
  
  def nBits = 2* n_ways //Considering 2 bits for each ways
  def perSet = true
  private val state_reg = RegInit(0.U(nBits.W))//Width of state register will be equals to the 2*nways
  def state_read = WireDefault(state_reg)//making input to read the state_reg
  
  private val replace1 = Wire(Bool())
  replace1 := false.B
  private val lfsr = LFSR(16, replace1)
  
  private def findMaxValueAndIndex(wayIndex: Seq[UInt]): (UInt, UInt) = {
    wayIndex.zipWithIndex.foldLeft((wayIndex.head, 5.U)) {
      case ((maxVal, maxIdx), (currentVal, currentIdx)) =>
        val newMaxVal = Wire(UInt())
        val newMaxIdx = Wire(UInt())
        when(currentVal >= maxVal){
          when(currentVal === maxVal){
            newMaxVal := currentVal
            newMaxIdx := Random(n_ways, lfsr)
          } .otherwise{
            newMaxVal := currentVal
            newMaxIdx := currentIdx.U  
          }
        } .otherwise {
          newMaxVal := maxVal
          newMaxIdx := maxIdx
  ...
  def get_next_state(state: UInt, touch_way: UInt): UInt = {
    val newState = Wire(UInt(state.getWidth.W))
    val wayIdx = (2.U * (touch_way)) 
    val wayState = (state>>wayIdx)(1,0)
    val updatedWayState = Mux(wayState === 0.U(2.W), 0.U(2.W), wayState - 1.U)
    assert(updatedWayState.getWidth == 2, "Width mismatch in updatedWayState")
    when(wayIdx === 0.U) {
      newState := Cat(state>>(wayIdx + 2.U), updatedWayState)
      assert(newState.getWidth == (2 * n_ways), "Width mismatch in newState first way")
    }.otherwise {
      newState := Cat(state>>(wayIdx + 2.U), updatedWayState, 0.U(wayIdx)) | ((state<<(state.getWidth.U-wayIdx))(state.getWidth - 1, 0))>>(state.getWidth.U-wayIdx)
      assert(newState.getWidth == (2 * n_ways), "Width mismatch in newState non-first way")
    }
    printf(s"BeforeState: $state   AfterState: $newState")
    newState
  }
  ...
  def access(touch_ways: Seq[Valid[UInt]]): Unit = {
    assert(1.U == 3.U, "It entered access2")
 ...
 def get_replace_way(state: UInt): UInt = {
  val waystate = VecInit.tabulate(n_ways)(i => state(2 * i + 1, 2 * i))
  val (maxWayState, maxIndex): (UInt, UInt) = findMaxValueAndIndex(waystate)
  val newState = waystate.map(x => x + 3.U-maxWayState)
  state_reg :=  newState.zipWithIndex.tail.foldLeft((newState.head,0)) { case ((pe,pi),(ce,ci)) => (Cat(pe,ce), ci) }._1
  maxIndex
  }

  def way = {
    get_replace_way(state_reg)
  }
  def miss = { 
    replace1 := true.B
    val wayRep = way
  ...
  def hit = {
    assert(1.U == 3.U, "It entered hit")
    }
  @deprecated("replace 'replace' with 'way' from abstract class ReplacementPolicy","Rocket Chip 2020.05")
  def replace: UInt = { assert(1.U == 3.U, "It entered replace")
    way}
}


//This is MRU Policy named as TrueLRU. To run MRU uncomment this part of code and comment all the other classes name TrueLRU
/*
class TrueLRU(n_ways: Int) extends ReplacementPolicy {
  ...
  def get_replace_way(state: UInt): UInt = {
    ...
    val mruWayDec     = (0 until n_ways).map { i =>         
      val upperMoreRecent = (if (i == n_ways-1) true.B else (~moreRecentVec(i).apply(n_ways-1,0)).andR)
      val lowerMoreRecent = (if (i == 0)        true.B else moreRecentVec.take(i).map(e => e(i)).reduce(_ && _))
...
//This is TrueLRU Policy named as TrueLRU. To run TrueLRU uncomment this part of code and comment all the other classes name TrueLRU
/*
class TrueLRU(n_ways: Int) extends ReplacementPolicy {
  def nBits = (n_ways * (n_ways-1)) / 2
  def perSet = true
